...
export default function App() {
  ...
  const renderCreate = () => {
    const stepContent = [
      ...
      // Step 2: Ad Copy (single copy for all ads)
      () => (
        <div style={S.card}>
          <div style={S.cardT}>Ad Copy</div>
          <div style={S.cardD}>This copy is used for ALL ads across all ad sets. Each uploaded video becomes a separate ad.</div>
          ...
          <div style={{marginTop:14,padding:10,background:"rgba(45,127,249,.05)",borderRadius:8,border:"1px solid rgba(45,127,249,.12)",fontSize:11.5,color:T.txM}}>
            {creatives.length} video{creatives.length!==1?"s":""} uploaded → {creatives.length} ad{creatives.length!==1?"s":""} will be created in each ad set, across all selected accounts.
          </div>
        </div>
      ),
